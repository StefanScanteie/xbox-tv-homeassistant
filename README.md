...
- Cloud commands go to Xbox Network remote management (`xccs.xboxlive.com`) as `sourceId` `com.microsoft.smartglass` — TurnOff, GoHome (return to Dashboard), ActivateApplicationWithOneStoreProductId (launch an installed app or game by its store product ID), InjectKey, Play/Pause/Next/Previous. Those are the same command types the Xbox mobile app and [xbox-webapi](https://github.com/OpenXbox/xbox-webapi-python) / Home Assistant `xbox` use for console control. Do **not** launch Dashboard as if it were a store app; that opens the Xbox “Something went wrong / Reload This Page” screen.
...
**Switching inputs** (Microsoft sign-in required):

- **An app or game** launches with that title’s Microsoft Store product ID (`ActivateApplicationWithOneStoreProductId`). If the console is off, the integration wakes it first.
- **Dashboard** sends `GoHome` only. That is the Xbox Network “return to home” command, not an app launch.
- If the console is off and you pick a source, it wakes, then switches.
...
Apple Home  →  HA HomeKit accessory (TV only)
                 →  media_player + Occupied sensor
                      ...
                      →  Xbox Network REST (off, GoHome, launch apps/games, remote, catalog)
```
...
| Changing source fails with `trailing comma` in `en.json` | HACS is still on 0.2.0. Update Xbox TV to **0.2.6** or later, restart, then try again |
| Changing source does nothing on the Xbox | Update to **0.2.6** or later. Apps/games use the store product launch command; Dashboard uses `GoHome`. Console must be signed in; if it is off, the integration wakes it first |
| Xbox shows “Something went wrong” / Reload This Page after Dashboard | 0.2.5 launched Dashboard as an app. Update to **0.2.6** or later. Press the Xbox button to leave the error page |
